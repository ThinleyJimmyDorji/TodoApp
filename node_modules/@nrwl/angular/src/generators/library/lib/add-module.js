...
function addModule(host, options) {
    if (options.routing && options.lazy) {
        add_lazy_loaded_router_configuration_1.addLazyLoadedRouterConfiguration(host, options);
    }
    if (options.routing && options.lazy && options.parentModule) {
        add_load_children_1.addLoadChildren(host, options);
    }
    if (options.routing && !options.lazy) {
        add_router_configuration_1.addRouterConfiguration(host, options);
    }
    if (options.routing && !options.lazy && options.parentModule) {
        add_children_1.addChildren(host, options);
    }
}
