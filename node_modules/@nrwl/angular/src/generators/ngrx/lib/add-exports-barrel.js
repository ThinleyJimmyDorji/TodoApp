...
function addExportsToBarrel(tree, options) {
    ...
    const indexFilePath = devkit_1.joinPathFragments(path_1.dirname(options.module), '..', 'index.ts');
    if (!tree.exists(indexFilePath)) {
        return;
    }
    const indexSourceText = tree.read(indexFilePath, 'utf-8');
    let sourceFile = typescript_1.createSourceFile(indexFilePath, indexSourceText, typescript_1.ScriptTarget.Latest, true);
    ...
    sourceFile = ast_utils_1.addGlobal(tree, sourceFile, indexFilePath, options.barrels
    ...
    if (options.barrels) {
        sourceFile = ast_utils_1.addGlobal(tree, sourceFile, indexFilePath, `export { ${className}Actions, ${className}Feature, ${className}Selectors };`);
    }
    if (options.syntax === 'creators') {
        sourceFile = ast_utils_1.addGlobal(tree, sourceFile, indexFilePath, `export * from '${statePath}.models';`);
    }
    if (options.facade) {
        sourceFile = ast_utils_1.addGlobal(tree, sourceFile, indexFilePath, `export * from '${statePath}.facade';`);
    }
}
