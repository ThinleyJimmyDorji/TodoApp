...
function ngrxGenerator(tree, options) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        const normalizedOptions = lib_1.normalizeOptions(options);
        if (!tree.exists(normalizedOptions.module)) {
            throw new Error(`Module does not exist: ${normalizedOptions.module}.`);
        }
        if (!normalizedOptions.onlyEmptyRoot ||
            (!normalizedOptions.root && normalizedOptions.minimal)) {
            lib_1.generateNgrxFilesFromTemplates(tree, normalizedOptions);
        }
        if (!normalizedOptions.onlyAddFiles) {
            lib_1.addImportsToModule(tree, normalizedOptions);
            lib_1.addExportsToBarrel(tree, normalizedOptions);
        }
        let packageInstallationTask = () => { };
        if (!normalizedOptions.skipPackageJson) {
            packageInstallationTask = lib_1.addNgRxToPackageJson(tree);
        }
