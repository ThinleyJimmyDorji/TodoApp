...
function getDecoratorMetadata(source, identifier, module) {
    const angularImports = find_nodes_1.findNodes(source, ts.SyntaxKind.ImportDeclaration)
    ...
    return get_source_nodes_1.getSourceNodes(source)
        .filter((node) => {
...
function _addSymbolToNgModuleMetadata(host, source, ngModulePath, metadataField, expression) {
    ...
    if (matchingProperties.length == 0) {
        ...
        return ast_utils_1.insertChange(host, source, ngModulePath, position, toInsert);
    }
...
function removeFromNgModule(host, source, modulePath, property) {
    ...
    if (matchingProperty) {
        return ast_utils_1.removeChange(host, source, modulePath, matchingProperty.getStart(source), matchingProperty.getFullText(source));
    }
}
...
function addImportToTestBed(host, source, specPath, symbolName) {
    const allCalls = (find_nodes_1.findNodes(source, ts.SyntaxKind.CallExpression));
    ...
    if (configureTestingModuleObjectLiterals.length > 0) {
        const startPosition = configureTestingModuleObjectLiterals[0]
            ...
            .getEnd();
        return ast_utils_1.insertChange(host, source, specPath, startPosition, `imports: [${symbolName}], `);
    }
    return source;
}
...
function addRoute(host, ngModulePath, source, route) {
    ...
    if (routes.hasTrailingComma || routes.length === 0) {
        return ast_utils_1.insertChange(host, source, ngModulePath, routes.end, route);
    }
    else {
        return ast_utils_1.insertChange(host, source, ngModulePath, routes.end, `, ${route}`);
    }
            ...
            if (e.kind === ts.SyntaxKind.CallExpression) {
                ...
                if ((text === 'RouterModule.forRoot' ||
                    ...
                    if (routes.kind === ts.SyntaxKind.ArrayLiteralExpression) {
                        ...
                        const variableDeclarations = find_nodes_1.findNodes(source, ts.SyntaxKind.VariableDeclaration);
...
function readBootstrapInfo(host, app) {
    const config = devkit_1.readProjectConfiguration(host, app);
    let mainPath;
    try {
